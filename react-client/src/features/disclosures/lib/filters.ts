...
export function getVisaFilters(
  ...
  return getEnumFiltersFromStrArray(columnFilters, "visaClass", VISA_CLASS_ENUM_TO_READABLE);
}

export function getCaseStatusFilters(
  columnFilters: ColumnFiltersState
): Casestatus[] | undefined {
  return getEnumFiltersFromStrArray(columnFilters, "caseStatus", CASE_STATUS_ENUM_TO_READABLE);
}
...
export function getEmployerNameFilters(
  columnFilters: ColumnFiltersState
): string[] | undefined {
...
export function getFacetedUniqueValuesMapFromArray<T extends {count: number}>(propertyKey: string, uniqueFacetsArray: T[], enumToStrRecord: Record<any, string>): Map<string, number> {
  return new Map(uniqueFacetsArray.map((facet) => {
